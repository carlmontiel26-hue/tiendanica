...
export default function Admin(){
  ...
  const saveProduct = async(e)=>{
    ...
    setUploading(true)
    try{
      let finalUrl=pform.image_url; if(imageFile) finalUrl=await uploadOne(imageFile)
      ...
      const payload={store_id:selectedStore.id,name:pform.name,price:parseFloat(pform.price),image_url:finalUrl,is_active:true,extra_images:selectedStore.tipo_tienda==='boutique'?extras:[],tallas:selectedStore.tipo_tienda==='boutique'?tallas:[],tipo_talla:tipoTalla}
      if(editing) await supabase.from('products').update(payload).eq('id',editing.id)
      else await supabase.from('products').insert(payload)
      setPform({name:'',price:'',image_url:''}); setImageFile(null); setExtra1(null); setExtra2(null); setTallas([]); setEditing(null)
  ...
  const delStore=async(id)=>{if(!confirm('Eliminar tienda y productos?'))return; await supabase.from('stores').delete().eq('id',id); setSelectedStore(null); loadStores()}
        ...
        <div className="grid lg:grid-cols-3 gap-6 mt-8">
          <div className="space-y-6">
            <div className="bg-white border rounded-2xl p-5">
              ...
              <form onSubmit={async(e)=>{e.preventDefault(); const slug=form.slug.toLowerCase().replace(/[^a-z0-9-]/g,'-'); const {data}=await supabase.from('stores').insert({name:form.name,slug,whatsapp:form.whatsapp,description:form.description,is_active:true}).select().single(); if(data){loadStores(); setSelectedStore(data)}}} className="mt-4 space-y-3">
              ...
              <div className="mt-3 space-y-2">
                {stores.map(s=>(
                  <div key={s.id} className={'border rounded-xl px-4 py-3 flex justify-between items-center '+(selectedStore?.id===s.id?'bg-black text-white':'bg-white')}>
                    ...
                    <div className="flex gap-1"><a href={'/'+s.slug} target="_blank" className="text-[10px] bg-white/20 px-2 py-1 rounded-full">Ver</a><button onClick={()=>delStore(s.id)} className="text-[10px] bg-red-500 text-white px-2 py-1 rounded-full">🗑️</button></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-2 bg-white border rounded-2xl p-6">
            <h2 className="font-black">Productos {selectedStore?.name} {isBoutique&&<span className="bg-black text-white text-[10px] px-2 py-1 rounded-full ml-2">BOUTIQUE CARRUSEL 3 FOTOS</span>}</h2>
            {selectedStore&&<>
              <form onSubmit={saveProduct} className="grid md:grid-cols-3 gap-3 mt-4 bg-gray-50 p-4 rounded-2xl border-2 border-dashed">
                ...
                <label className="md:col-span-3 border-2 border-dashed rounded-xl px-3 py-4 text-center bg-white cursor-pointer"><span className="text-sm font-bold">{imageFile?'✅ '+imageFile.name: editing?'Cambiar Foto 1 Frente':'📸 Foto 1 Frente'}</span><input type="file" accept="image/*" className="hidden" onChange={e=>setImageFile(e.target.files[0])}/></label>
                {isBoutique&&<>
                  ...
                  <div className="md:col-span-3 bg-white border rounded-xl p-3">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">{['S','M','L','XL','XXL','28','30','32','34'].map(t=><button type="button" key={t} onClick={()=>setTallas(p=>p.includes(t)?p.filter(x=>x!==t):[...p,t])} className={`min-w-[44px] px-3 py-2 rounded-full text-xs font-bold border ${tallas.includes(t)?'bg-black text-white':'bg-white'}`}>{t}</button>)}</div>
                    <input value={tallas.join(', ')} readOnly className="mt-2 w-full border rounded-full px-3 py-2 text-xs bg-black/5" placeholder="Tallas"/>
                  </div>
                </>}
                <button disabled={uploading} className="md:col-span-3 bg-[#00D084] text-black py-3 rounded-full font-bold">{uploading?'Guardando...': editing?'Actualizar':'Agregar Producto'}</button>
              </form>
