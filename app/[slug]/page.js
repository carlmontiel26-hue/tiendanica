...
export default function Tienda({params}){
 ...
 const cover=store.cover_image || prods[0]?.image_url || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=1200'
 ...
 <main className={`${isBoutique?'bg-[#F6F3F0] text-black':'bg-[#0A0A0A] text-white'} min-h-screen`}>
  ...
  {/* PORTADA GRANDE RESTAURADA - ESTO ERA LO QUE FALTABA */}
  <div className="relative h- overflow-hidden">
    ...
    <div className="absolute bottom-6 left-4 right-4">
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded- p-5 shadow-2xl">
        <h1 className="font-black uppercase text- leading-tight text-white tracking-tight">{store.name}</h1>
        <p className="text- text-white/70 mt-1">{store.description}</p>
        {isComida?(
          <div className="flex gap-2 mt-4">
            <button className="flex-1 bg-white text-black py-3 rounded-full font-black text-">🍽️ Menú</button>
            <button onClick={()=>setShowBebidas(true)} className="flex-1 bg-[#00E676] text-black py-3 rounded-full font-black text-">🥤 Bebidas y Extras {bebidas.length?`(${bebidas.length})`:''}</button>
          </div>
        ): isBoutique?(
          <div className="mt-3"><span className="bg-black text-white text- font-black px-4 py-2 rounded-full">👗 Nueva Colección • {prods.length} prendas</span></div>
        ):(
          <div className="mt-3"><span className="bg-white text-black text- font-black px-4 py-2 rounded-full">📱 {prods.length} productos</span></div>
        )}
      </div>
    </div>
  </div>
