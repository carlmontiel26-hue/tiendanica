...
export default function Tienda({params}){
 ...
 <main className={`${isBoutique?'bg-[#F6F3F0] text-black':'bg-[#0A0A0A] text-white'} min-h-screen`}>
  <div className="fixed bottom-6 right-4 z-[100] flex flex-col items-end gap-2">
   {showCart&&cart.length>0&&<div className={`${isBoutique?'bg-white border border-black/10':'bg-white text-black'} rounded- p-4 w- max-w- shadow-2xl`}><div className="flex justify-between mb-2"><b>Pedido ({count})</b><button onClick={()=>setShowCart(false)}>✕</button></div>{cart.map(i=><div key={i.cartId} className="flex gap-2 bg-black/5 rounded-xl p-2 mb-1"><img src={i.image_url} className="w-10 h-10 rounded object-cover"/><div className="flex-1 text-xs"><b className="truncate">{i.name}</b><br/>{i.qty}x C$ {i.price}{i.tallaSel?' • '+i.tallaSel:''}</div></div>)}<div className="border-t mt-2 pt-2 flex justify-between font-black"><span>Total</span><span>C$ {total}</span></div><button onClick={sendWA} className="mt-3 w-full bg-[#00E676] py-3 rounded-full font-black">WhatsApp</button></div>}
  ...
  <div className="relative h- overflow-hidden">
    ...
    <div className="absolute bottom-4 left-4 right-4 bg-white/10 backdrop-blur border border-white/20 rounded- p-4">
      <h1 className="font-black uppercase text- text-white">{store.name}</h1>
      <p className="text- text-white/70">{store.description}</p>
      {isComida?(
        <div className="flex gap-2 mt-3">
          <button className="flex-1 bg-white text-black py-2.5 rounded-full font-black text-">🍽️ Menú</button>
          <button onClick={()=>setShowBebidas(true)} className="flex-1 bg-[#00E676] text-black py-2.5 rounded-full font-black text-">🥤 Bebidas y Extras {bebidas.length?`(${bebidas.length})`:''}</button>
        </div>
      ): isBoutique?(
        <span className="mt-2 inline-block bg-black text-white text- font-black px-3 py-1 rounded-full">👗 Nueva Colección • {prods.length} prendas</span>
      ):(
        <span className="mt-2 inline-block bg-white text-black text- font-black px-3 py-1 rounded-full">📱 {prods.length} productos disponibles</span>
      )}
    </div>
  </div>
  ...
  <div className="max-w-6xl mx-auto p-4 pb-28">
   ...
   {isBoutique&&<div className="mb-6 bg-black text-white rounded- p-4"><p className="font-black text-sm">✨ Casa Lino Nicaragua</p><p className="text-xs opacity-70">Toca una prenda para ver sus 3 fotos y elegir talla</p></div>}
